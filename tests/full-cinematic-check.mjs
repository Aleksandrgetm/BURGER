...
const metadata=await page.locator('video[src]').evaluateAll(vs=>vs.map(v=>({src:v.getAttribute('src'),duration:v.duration,width:v.videoWidth,height:v.videoHeight,time:v.currentTime})))
...
await seek(.94)
const heldTime=await page.locator('.scene-final video').evaluate(v=>v.currentTime)
await seek(.98);await page.waitForTimeout(600)
if(await page.locator('.scene-final video').evaluate(v=>v.currentTime)!==heldTime)errors.push('Final scroll hold failed')
if(await page.locator('.cta-buttons').evaluate(el=>+getComputedStyle(el).opacity)!==1)errors.push('Final CTAs hidden')
await page.getByRole('button',{name:'ORDER NOW',exact:false}).click();await page.getByRole('button',{name:'Close dialog'}).click()
await page.getByRole('button',{name:'FIND A LOCATION',exact:true}).click();await page.getByRole('button',{name:'Close dialog'}).click()
...
await page.getByRole('navigation',{name:'Footer navigation'}).getByRole('button',{name:'MENU',exact:true}).click();await page.getByRole('button',{name:'Close dialog'}).click()
for(const p of [1,.94,.86,.76,.64,.56,.38,.16,0])await seek(p)
...
for(const viewport of [{width:375,height:667},{width:844,height:390}]){
 await page.setViewportSize(viewport);await page.waitForTimeout(450);await seek(.97)
 await page.screenshot({path:`/tmp/burgr-final-${viewport.width}.png`})
 const rect=await page.locator('.cta-buttons').evaluate(el=>{const r=el.getBoundingClientRect();return {top:r.top,bottom:r.bottom}})
 if(rect.top<70||rect.bottom>viewport.height-55)errors.push({message:'Final CTA clipping',viewport,rect})
}
await seek(0)
